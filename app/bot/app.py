...
from app.bot.assets.answer_to_question import get_answer
from app.models import Account, TGAccount, VKAccount
...
class NissanBot:
    ...
    def create_longpoll_listeners(self):
        logger.debug("create_longpoll_listeners")
    ...
    def _create_listeners_tg(self, token):
        ...
        async def _(call: aiogram.types.CallbackQuery):
            logger.debug("_callback_query_handler")
    ...
    def _create_listeners_vk(self, token):
        logger.debug("_create_listeners_vk")
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        ...
        async def _(message):
            logger.debug("___vkbot.on.message")
        ...
        async def _(event: GroupTypes.MessageEvent):
            logger.debug("__vkbot.on.raw_event")
    ...
    async def handle_message(self, message, user, bot_object):
        ...
        logger.debug(f"processed_name: {processed_name}")
        if ('меню' in processed_name or '/start' in processed_name) and user.dialog != Account.Dialog.START:
            logger.debug("in if")
            await user.return_menu()
        else:
            logger.debug("in else1")
            for command in handler.commands:
                # res = command.handle(message, path_args, bot_object, user)
                # logger.debug(f"command.only_to: {command.only_to}")
                # logger.debug(f"message.class_name: {message.class_name}")
                # logger.debug(f"command.with_args: {command.with_args}")
                # logger.debug(f"command.name: {command.name}")
                # logger.debug(f"command.dialog: {command.dialog}")
                # logger.debug(f"user.dialog: {user.dialog}")
                ...
                logger.debug("in else2")
                faq = get_answer(message.text.strip())
                if faq is None:
                    await message.reply('⚠️ Неизвестная команда или вопрос. Напишите мне <b>«Меню»</b> и воспользуйтесь кнопками')
                else:
